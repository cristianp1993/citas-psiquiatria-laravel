...
class PublicController extends Controller {
    public function index() {
        $doctors = Doctor::where('is_active',true)->with('specialty')->get(['id','name','slug','specialty_id']);
        return Inertia::render('Public/Index', ['doctors'=>$doctors]);
    }

    public function doctor(Doctor $doctor) {
        return Inertia::render('Public/Doctor', ['doctor'=>$doctor]);
    }
    ...
    public function store(Request $request) {
        ...
        if(!$available) return back()->withErrors(['start_at'=>'Horario no disponible']);
