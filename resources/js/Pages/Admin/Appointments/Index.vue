    ...
    overlay.innerHTML = `
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 max-w-sm w-full mx-4">
        ...
        <div class="flex justify-end space-x-2">
          <button class="px-4 py-2 rounded border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700" id="cancel-btn">
            Cancelar
          </button>
          <button class="px-4 py-2 rounded bg-red-600 text-white hover:bg-red-700" id="confirm-btn">
            Confirmar
          </button>
        </div>
      </div>
...
async function accept(appointment) {
  const confirmed = await customConfirm('¿Aceptar esta cita?')
  if (confirmed) {
    router.post(route('appointments.accept', appointment.slug), {}, {
      onSuccess: () => {
        Toastify({
          text: "Cita aceptada correctamente",
          duration: 3000,
          gravity: "top",
          position: "right",
...
async function reject(appointment) {
  const confirmed = await customConfirm('¿Rechazar esta cita?')
  if (confirmed) {
    router.post(route('appointments.reject', appointment.slug), {}, {
      ...
      onError: (errors) => {
        Toastify({
          text: "Error al rechazar la cita",
          duration: 3000,
          gravity: "top",
          position: "right",
          backgroundColor: "#dc2626",
          ...
          <div class="p-6 text-gray-900 dark:text-gray-100">
              ...
              <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                <tr v-for="appointment in appointments.data" :key="appointment.id">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    ...
                    <Link 
                      :href="route('appointments.destroy', appointment.slug)" 
                      method="delete" 
                      as="button" 
                      class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300"
                      @click.prevent="() => {
                        customConfirm('¿Eliminar esta cita?').then(confirmed => {
                          if (confirmed) {
                            router.delete(route('appointments.destroy', appointment.slug), {
                              onSuccess: () => {
                                Toastify({
                                  text: 'Cita eliminada',
                                  duration: 3000,
            ...
            <div class="mt-4" v-if="appointments.last_page > 1">
              <Link 
                ...
                :key="page" 
                :href="route('appointments.index', { ...filters, page })" 
                :class="['px-3 py-2 mx-1 border rounded', page == appointments.current_page ? 'bg-blue-500 text-white' : 'bg-white text-blue-500']"
              >
                {{ page }}
              </Link>
            </div>
